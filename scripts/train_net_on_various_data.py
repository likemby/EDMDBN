...
DATASETS = {
    'casme2': {
        3: (
            {
                'the_whole_face': '/home/mby/computer_vision/database/CASME2/crop_the_whole_face',
                'crop_face_two_sides':'/home/mby/computer_vision/database/CASME2/crop_face_two_sides'
            },
    ...
    'samm': {
        3: (
            {
                'the_whole_face': '/home/mby/computer_vision/database/SAMM/crop_the_whole_face',
                'crop_face_two_sides':'/home/mby/computer_vision/database/SAMM/crop_face_two_sides',
                'align_the_whole_face':'/home/mby/computer_vision/database/SAMM/align_the_whole_face',
                'align_crop_face_two_sides':'/home/mby/computer_vision/database/SAMM/aligned_dlib_crop_face_v3'
        ...
        'smic-hs-e': {
        3: ({
                'crop_face_two_sides':'/home/mby/computer_vision/database/SMIC-HS-E/dlib_crop_face_v3',
                'align_the_whole_face':'/home/mby/computer_vision/database/SMIC-HS-E/aligned_dlib_crop_twice',
                'align_crop_face_two_sides':'/home/mby/computer_vision/database/SMIC-HS-E/aligned_dlib_crop_face_v3'
        },
            '/home/mby/computer_vision/database/SMIC-HS-E/unique_path_data_annotations/3classes-loso-annotations'),
    },
    'mmew':{
         3: (
             {'the_whole_face':'/home/mby/computer_vision/database/MMEW/Micro_Expression_unique_path',},
...
def run_commands(args):
    ...
    for dataset in args.datasets:
        for num_classes in args.num_classes:
            ...
            data_processed, annotation_path = DATASETS[dataset][num_classes]
            ...
            data_root =data_processed.get(args.processing_method, None)
            if data_root is None:
                print(f"Error: {this_purpose}, {args.processing_method} is not supported!")
                exit(1)
            ...
            with open(log_file_path, 'w') as f:
                flag = subprocess.Popen(
                    command, stdout=f, stderr=subprocess.STDOUT).wait()
            if flag != 0:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Python version of the bash script", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('remark', type=str, help='First argument: remark')
    parser.add_argument('--datasets', nargs='+', default=[
                        'all'], help='Third argument: dataset names (multiple values allowed)', choices=list(DATASETS.keys())+['all'])
    parser.add_argument('--num_classes', nargs='+', default=[
                        0], type=int, help='Fourth argument: num_classes (multiple values allowed)', choices=[3, 4, 5]+[0])
    parser.add_argument('--fast_dev_run', action='store_true',
                        default=False, help='Fifth argument: fast_dev_run (optional)')
    parser.add_argument('--use_offline_aug', default=False,
                        action="store_true", help="启用离线数据增强")
    parser.add_argument('--use_class_weight', action='store_true')
    parser.add_argument('--processing_method', type=str,  help='选择数据处理方式', choices=['the_whole_face','crop_face_two_sides','align_the_whole_face','align_crop_face_two_sides'])
    parser.add_argument('--batch_size', type=int, default=4,
                        help='Sixth argument: batch_size (optional)')
    parser.add_argument('--weight_decay', type=float,
                        default=1e-8, help='weight_decay (optional)')
    parser.add_argument('--use_pos_vit', action='store_true')

    # 对比实验参数
    # [short action]
    parser.add_argument('--sampling_strategy', type=int, default=3,
                        help='Second argument: sampling_strategy', choices=[2, 3])
    parser.add_argument('--use_swin_CA', action='store_true')

    # [long action]
    parser.add_argument('--rgb_diff_abs',  action='store_true',
                        default=False, help='abs(rgb_diff)')
    parser.add_argument('--block_name', type=str, default='CABlock',
                        choices=['CBAMBlock', 'BasicBlock', 'CABlock'])

    # [Dataset parameters] 0: 前后帧相减，1：后帧减第一帧 2：保持像素帧
    parser.add_argument('--frames_mode',  default=0,
