...
DATASETS = {
    'megc2019cd': {
        3:'/home/mby/computer_vision/database/megc2019cd/unique_path_data_annotations/3classes-loso-annotations'
    }
}
    ...
    for dataset in args.datasets:
        for num_classes in args.num_classes:
            if num_classes not in DATASETS[dataset]:
                continue
            annotation_path = DATASETS[dataset][num_classes]
...
def generate_command(args,num_classes,dataset, data_root, annotation_path, purpose, log_dir_name, base_config_file):
    ...
    if args.only_use_long_action:
        command.remove("--use_short_action")
    if args.only_use_short_action:
        command.remove("--use_long_action")
...
def parse_args():
    ...
    # 对比实验参数
    # [short action]
    parser.add_argument('--only_use_short_action', action='store_true')
    ...
    # [long action]
    parser.add_argument('--only_use_long_action', action='store_true')
